...
void canRead() {
  ...
  if (rxId == 0X60A) {
    ...
    Serial.print(b6);
    Serial.println(b7);
    CAN0.sendMsgBuf(0x58A, 0, 8, txBuf);
  }
}
...
void SendButtonInfo() {
  ...
  bitWrite(ButtonInfo[0], 7, !digitalRead(4));                      // byte 0, bit 0, button 8
  ...
  CAN0.sendMsgBuf(0x18A, 0, 3, ButtonInfo);           // send the 3 byte data buffer at adres 18D
}


void SendKeepAlive() {                                // send keep alive frame
  byte KeepAlive[1] = { 5 };                          // frame dat is 0X05 for byte 0
  CAN0.sendMsgBuf(0x70A, 0, 1, KeepAlive);            // send the frame at 70D 
}
